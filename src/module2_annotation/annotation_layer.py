...
def phase_a_coordinate_correction(df: pd.DataFrame, logger: logging.Logger, output_path: str, af_threshold: float = 0.001) -> pd.DataFrame:
    ...
    logger.info(f"Phase A: Querying {len(rsids)} RSIDs for coordinate correction and AF")
    session = get_session()
    ...
    for i in range(0, len(rsids), batch_size):
        ...
        try:
            res = session.post(url, headers=headers, json={"ids": batch}, timeout=60)
            if res.status_code != 200:
                logger.warning(f"Failed batch {batch_num}: HTTP {res.status_code}")
                continue
                ...
                if ('gnomadg' in p_name or 'gnomade' in p_name) and pop.get('allele') == alt_allele:
                    ...
                    if freq is not None:
                        ...
                        if pd.isna(max_af) or f_val > max_af:
                            max_af = f_val
            af_map[rsid] = max_af
    logger.info(f"Phase A: Mapped coordinates and AF for {len(coord_map)} RSIDs")
    df = df.copy()
    ...
    logger.info(f"AF==0.0: {zero_count} ({zero_count/len(df)*100:.2f}%), null: {null_count}, "
                f"min={df['gnomAD_AF'].min()}, max={df['gnomAD_AF'].max()}, mean={df['gnomAD_AF'].mean()}")
    if (zero_count / len(df)) > 0.80:
        logger.warning("AF unreliable: >80% of values are 0.0")
        print("WARNING: AF unreliable. Stopping execution.")
        sys.exit(1)
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    original_count = len(df)
    df = df[(df['gnomAD_AF'].isna()) | (df['gnomAD_AF'] < af_threshold)].copy()
    logger.info(f"Phase A: Removed {original_count - len(df)} variants above AF threshold ({af_threshold})")
    df.to_csv(output_path, index=False)
    logger.info(f"Phase A: Saved to {output_path}")
    return df

def fetch_uniprot_features(uniprot_id: str, logger: logging.Logger) -> Dict[str, Any]:
    url = f"https://rest.uniprot.org/uniprotkb/{uniprot_id}.json"
    logger.info(f"Phase B: Fetching UniProt record for {uniprot_id}")
    session = get_session()
    domains = {}
    try:
        ...
        for f in data.get('features', []):
            ...
            if t == 'Signal':
                ...
                transmem_count += 1
        logger.info(f"Phase B: Domain boundaries: {domains}")
    except Exception as e:
        logger.warning(f"Phase B: Failed to parse UniProt domains for {uniprot_id}: {e}")
    return domains
...
def phase_b_structural_domains(df: pd.DataFrame, logger: logging.Logger, config_path: str, config: dict) -> pd.DataFrame:
    ...
    logger.info(f"Phase B: Updated {config_path} with domain boundaries")
...
def phase_c_functional_scores(df: pd.DataFrame, logger: logging.Logger, output_path: str, config: dict) -> pd.DataFrame:
    ...
    vep_map = {}
    alt_map = dict(zip(df['rsid'], df['alt']))
    logger.info(f"Phase C: Querying {len(rsids)} RSIDs to VEP for CADD/SIFT/PolyPhen")
    session = get_session()
    total_batches = (len(rsids) + batch_size - 1) // batch_size
    for i in range(0, len(rsids), batch_size):
        batch_num = (i // batch_size) + 1
        ...
        batch = rsids[i:i + batch_size]
        try:
            res = session.post(f"{url}?CADD=1", headers=headers, json={"ids": batch}, timeout=120)
            if res.status_code != 200:
                logger.warning(f"Failed batch {batch_num}: HTTP {res.status_code}")
                continue
            res.raise_for_status()
        ...
        for item in data:
            ...
            cadd_phred = sift = sift_pred = polyphen = polyphen_pred = None
            has_match = False
    ...
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    df.to_csv(output_path, index=False)
    logger.info(f"Phase C: Saved annotated CSV to {output_path}")
    print("\n========= MODULE 2 SUMMARY ==========")
    ...
    print(f"Transmembrane: {tm_perc:.2f}%  |  Pore (M2): {pore_perc:.2f}%")
