...
print(f"STEP 2 - Structural Metrics ({structure_id.upper()})")
...
chain_A = model['A']
ligand_atoms = [
    a for chain in model for res in chain for a in res
    if res.id[0] != ' ' and res.id[0] != 'W' and res.resname != 'HOH' and a.element != 'H'
]
other_chain_atoms = [
    a for chain in model for res in chain for a in res
    if chain.id != 'A' and res.id[0] == ' ' and a.element != 'H'
]
...
def get_metrics(row):
    ...
    if ligand_mode == "force_off" or len(ligand_atoms) == 0:
        ...
        min_ligand = min(
            (np.linalg.norm(ta.coord - la.coord) for ta in target_atoms for la in ligand_atoms),
            default=np.nan
        )
    min_interface = min(
        (np.linalg.norm(ta.coord - oa.coord) for ta in target_atoms for oa in other_chain_atoms),
        default=np.nan
    )
    radial_dist = np.nan
